...
global build command and dev dependencies on Rollup itself and all plugins
you're using. E.g.:
...
```json
{
  ...
  "devDependencies": {
    "@calmdown/rollup-workspace": "1.0.0",
...
Then, declare the plugins you will be using in `rollup-plugins.mjs`. To avoid
importing plugins that are only used by some targets, dynamic imports are
typically used. E.g.:

```js
import { declarePlugin } from "@calmdown/rollup-workspace";
...
Then, define common build targets in `rollup-targets.mjs`. E.g.:

```js
import { declareTarget, inEnv } from "@calmdown/rollup-workspace";
...
import { build } from "@calmdown/rollup-workspace";
